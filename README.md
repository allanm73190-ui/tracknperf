...
```
src/
├── domain/          # Logique métier pure (moteur adaptatif, types, règles)
│   └── engine/      # Moteur adaptatif v1.1 (3 couches : rules → adaptive → optimization)
├── application/     # Use-cases : orchestration domain ↔ infra, sans React
├── infra/           # Adapters externes (Supabase client, IndexedDB, sync queue)
├── auth/            # Hooks d'authentification React
├── ui/              # Composants et pages React (consomme application/, jamais domain/ direct)
│   ├── pages/
│   └── components/
└── app/             # Entrée React (Router, providers)
```

**Règle d'or :** `ui/` ne doit jamais importer directement depuis `infra/` ou `domain/` — passer par `application/`.
